...
def _toc_run(p, text, size_pt, bold):
    r = p.add_run(text); font_run(r, size_pt, bold)
    r.font.color.rgb = RGBColor(0, 0, 0); r.font.underline = False   # 不要蓝色下划线
    return r

def toc_entry(doc, style_name, text, bm_name, size_pt, bold):
    """目录行: 整行包 w:hyperlink 锚到正文书签(可点跳转) + 文字(字号写死) + 点引线 + PAGEREF 页码。"""
    p = doc.add_paragraph(style=style_name)
    runs = []
    runs.append(_toc_run(p, text, size_pt, bold))
    runs.append(_toc_run(p, '\t', size_pt, bold))
    rb = p.add_run(); rb._element.append(_fld('begin')); runs.append(rb)
    ri = p.add_run(); it = OxmlElement('w:instrText'); it.set(qn('xml:space'), 'preserve')
    it.text = f' PAGEREF {bm_name} \\h '; ri._element.append(it); runs.append(ri)
    rs = p.add_run(); rs._element.append(_fld('separate')); runs.append(rs)
    runs.append(_toc_run(p, '1', size_pt, bold))      # 占位页码, F9 后更新
    re_ = p.add_run(); re_._element.append(_fld('end')); runs.append(re_)
    # 把这些 run 移进一个锚到书签的 hyperlink, 实现整行可点跳转
    hyper = OxmlElement('w:hyperlink'); hyper.set(qn('w:anchor'), bm_name)
    for r in runs:
        el = r._element; el.getparent().remove(el); hyper.append(el)
    p._p.append(hyper)
    return p
