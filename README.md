...
一个**半自动**的"三周报"制作技能：按三周一期的节奏，把 **AI×人力资源、以及 OBHRM 各细分主题**领域的最新学术文献与权威实务报告，**检索 → 去重 → 归类 → 写中文摘要 → 生成统一格式 Word 文档**。运行在 **Claude Code**（也兼容 Codex / Antigravity 等支持 Agent Skills 的工具）。
...
- 文件开头即**目录**（带点引线和自动页码）；
- 正文按 **5 个（或你自定义的）板块**分组；
- 每条 = **中文标题 + 中文摘要 + 【原文信息】引文**；
- 格式复刻既有模板（宋体/Cambria、正文 12pt、行距 1.16、段后 8pt、摘要两端对齐等）。

## 二、边界：半自动，不是全自动（务必先理解）

- Claude **不会定时运行、不会主动提醒**，每期由人开口触发。
- 学术检索靠 **Crossref（确定、穷尽）** + 联网搜索（补实务报告）；**微信公众号/小红书无法自动枚举**，由你手动贴、Claude 回溯到其引用的原始论文。
...
3. **报本期主题 + 板块 + 检索深度**：AI×OBHRM（默认），或其他 OBHRM 主题（组织变革 / 招聘甄选 / 薪酬 / 团队动力学…）；给本期板块清单（可自拟、数目不限，不报则用默认）；并选**检索深度**——只顶刊 4*/4 / 含 3★ 中坚层（默认）/ 再含 2★ 具名刊（详见附录与下方说明）。
4. **检索**：Claude 跑 Crossref 拉 A 类全量候选 + 抓实务报告。
5. **上传社媒**：Claude 停下，请你贴微信公众号 / 小红书（没有就回"无"）；它回溯到原始论文再纳入。
6. **评审**：Claude 给每条"主旨 + 入选理由 + 置信度"，你定边缘条目留/删、新来源批/不批。
...
- **web_search 部分非确定/不穷尽**：A 类靠 Crossref 才能确定且拉全；社媒、实务报告无法穷尽。
- **完全无人值守不现实**：每期需人触发并在三处把关。
